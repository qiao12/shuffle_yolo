...
def edge2delta(proposals, edges, means=[0, 0], stds=[1, 1]):
    assert len(means) == len(stds) == 2
    assert proposals.size(0) == edges.size(0)
    ...
    pw = (proposals[..., 2] - proposals[..., 0] + 1.0).unsqueeze(1).expand_as(dx)
    ph = (proposals[..., 3] - proposals[..., 1] + 1.0).unsqueeze(1).expand_as(dy)

    deltas = edges.new_zeros(edges.size())
    deltas[:, 0::2] = (dx - px) / pw
    deltas[:, 1::2] = (dy - py) / ph
...
def delta2edges(rois,
    ...
    edges = denorm_deltas.new_zeros(denorm_deltas.size())
    ...
    edges[:, 0::2] = torch.addcmul(px, 1, pw, dx)
    edges[:, 1::2] = torch.addcmul(py, 1, ph, dy)
    return edges